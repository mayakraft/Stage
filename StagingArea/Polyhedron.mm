...
-(void) makeNormals{
    delete normalPoints;
    normalPoints = (float*)malloc(sizeof(float) * numTriangles*3);
    for(int i = 0; i < geodesic.numFaces; i++){
        // triangle vertex 1: X Y and Z
        normalPoints[i*9 + 0*3 + 0] = geodesic.normals[ geodesic.faces[0+i*3]*3 + X ];
        normalPoints[i*9 + 0*3 + 1] = geodesic.normals[ geodesic.faces[0+i*3]*3 + Y ];
        normalPoints[i*9 + 0*3 + 2] = geodesic.normals[ geodesic.faces[0+i*3]*3 + Z ];
        // triangle vertex 2: X Y and Z
        normalPoints[i*9 + 1*3 + 0] = geodesic.normals[ geodesic.faces[1+i*3]*3 + X ];
        normalPoints[i*9 + 1*3 + 1] = geodesic.normals[ geodesic.faces[1+i*3]*3 + Y ];
        normalPoints[i*9 + 1*3 + 2] = geodesic.normals[ geodesic.faces[1+i*3]*3 + Z ];
        // triangle vertex 3: X Y and Z
        normalPoints[i*9 + 2*3 + 0] = geodesic.normals[ geodesic.faces[2+i*3]*3 + X ];
        normalPoints[i*9 + 2*3 + 1] = geodesic.normals[ geodesic.faces[2+i*3]*3 + Y ];
        normalPoints[i*9 + 2*3 + 2] = geodesic.normals[ geodesic.faces[2+i*3]*3 + Z ];
    }
    NSLog(@"_____________________");
    NSLog(@"    NORMALS");
    NSLog(@"_____________________");
